...
# encryption key
gamma = [  0x15, 0xE7, 0x40, 0x82, 0x97, 0xFD, 0xA6, 0xF3, 0xFB, 0x05, 0x53, 0xAD, 0xD3, 0x99, 0xF1, 0x59, 0x18,
...
ciphertext = [[0x3a, 0x81, 0x21, 0xf4, 0xfe, 0x9e, 0xc9, 0x9d, 0xd5, 0x6c, 0x38, 0xc2],
              ...
              [0x24, 0xa5, 0x6c, 0xea, 0xe2, 0x87, 0xf5, 0xbd, 0xd4, 0x6f, 0x78, 0xce, 0xfb, 0xa5, 0xcb, 0x2a, 0x20, 0x18,
              0x45, 0x8b, 0x88, 0x2d, 0x8f, 0xc4, 0x43, 0xaa, 0x28, 0xf7, 0xd0, 0xb8, 0xfb, 0x88],
              ...
              [0x3f, 0xa0, 0x9, 0xc6, 0xde, 0xb9, 0x9b, 0xc5, 0xcc, 0x31, 0x66, 0x9b, 0xe3, 0xa0, 0xc9, 0x6e, 0x2e, 0x69, 0x2, 0xd3, 0xd3],
...
def get_plain(i):
    ...
    for k in range(0, lengths[i]):
        gam = 0x00
        if (k > (len(gamma) - 1)):
            gam = gamma[k % len(gamma)]
        else:
            gam = gamma[k]
        result.append(gam ^ arr[k])
    return result

def Main():
    print "This script decrypts and prints all encrypted strings which where"
    print "in the infected nginx sample (MD5: e3abf42c0a14cca173070d4da051f473)"
    print "Authors:"
